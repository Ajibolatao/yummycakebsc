...
const pcsRouter = require('../web3/pcsRouter')
...
const tikiContract = new ethers.Contract(DogeBNBContractAddress, tikiAbi, provider)
...
console.log(getChunksAndMergeThenReturnHighest(1, 86000, 3))
...
//gives dogeBNB price
async function getDOGEBNBPrice() {
  const functionResponse = await getAmountsOut(`${1 * Math.pow(10, DogeBNBDecimals)}`, [DogeBNBContractAddress, bnb.address, busd.address])
  const priceInUsd = Number(functionResponse.amounts[2].toString()) / Math.pow(10, busd.decimals)
  console.log('DOGEBNB price:', priceInUsd)
  return priceInUsd
}
...
//   const tikiContract = new ethers.Contract(DogeBNBContractAddress, tikiAbi, wallet)
